...
# from p2p_net.node import Node

from db_manager import DatabaseManager
# from p2p_net.db_manager import DatabaseManager
...
class BlockChainNode(Node):
    ...
    def inbound_node_connected(self, connected_node):
        '''외부 노드가 나에게 접속한 경우'''
        # print(f"\ninbound_node_connected: {connected_node.ip}:{connected_node.port}")
        ...
        self.add_node_to_node_list(connected_node)


    def inbound_node_disconnected(self, connected_node):
        '''외부에서 나한테 접속했던 노드가 끊긴 경우'''
        # print(f"\ninbound_node_disconnected: {connected_node.ip}:{connected_node.port}")
    ...
    def outbound_node_connected(self, connected_node:Node):
        '''내가 다른 노드로 접속한 경우 경우'''
        # print(f"\noutbound_node_connected: {connected_node.ip}:{connected_node.port}")
        # print(f'Me: {self.id[:10]} --> Outbound:{connected_node.id[:10]} ')
    ...
    def outbound_node_disconnected(self, connected_node):
        ...
        # print(f"outbound_node_disconnected: {connected_node.ip}:{connected_node.port}")
        node = self.db_manager.query_data(connected_node.ip, connected_node.port)
        self.set_is_active(node, False)
        self.remove_node_from_node_list(connected_node)
    ...
    def add_node_to_node_list(self, connected_node: Node) -> None:
        if connected_node not in self.nodes_inbound:
            self.nodes_inbound.append(connected_node)
            # print(f'inbound node removed: {connected_node.id[:10]}')
        if connected_node not in self.nodes_outbound:
            self.nodes_outbound.append(connected_node)
            # print(f'Outbound node removed: {connected_node.id[:10]}')
