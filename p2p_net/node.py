...
# from p2p_net.nodeconnection import NodeConnection
...
class Node(threading.Thread):
    ...
    provide a callback method. It is preferred to implement a new node by extending this Node class.
      ip: IP address that is used to bind the TCP/IP server to.
      port: The port number that is used to bind the TCP/IP server to.
      hostname: The host name that is used to bind the TCP/IP server to.
    ...
    def __init__(
        ...
        """Create instance of a Node. If you want to implement the Node functionality with a callback, you should
           provide a callback method. It is preferred to implement a new node by extending this Node class.
            ip: IP address that is used to bind the TCP/IP server to.
            port: The port number that is used to bind the TCP/IP server to.
            hostname: The host name or ip address that is used to bind the TCP/IP server to.
        ...
        # A list of nodes that should be reconnected to
        ...
        self.external_ip:str = utils.get_external_ip()
    ...
    def debug_print(self, message):
        ...
        if self.debug:
            ...
            # 해시값이 너무 길어서 앞 10자리만 표시하는 것으로 수정함 by Giseop 23. 6. 21.(월)
            print("DEBUG (" + self.id[:10] + "): " + message)

    def generate_id(self):
        """Generates a unique ID for each node."""
        id = hashlib.sha512()
        # t = self.ip + str(self.port) + str(random.randint(1, 99999999))
        # Node ID 생성 정보를 time 함수로 변경 by Giseop 23.06. 19.(월)
        t = str(self.ip) + str(self.port) + str(time.time())
        id.update(t.encode('ascii'))
        return id.hexdigest()

    def init_server(self):
        """Initialization of the TCP/IP server to receive connections. It binds to the given ip and port."""
        print("Initialization of the Node on port: " + str(self.port) + " on node (" + self.id + ")")
    ...
    def connect_with_node(self, ip, port, reconnect=False):
        """ Make a connection with another node that is
            running on ip with port.
            When the connection is made, an event is triggered
            outbound_node_connected.

            When the connection is made with the node,
            it exchanges the id's of the node.
            First we send our id and then we receive
            the id of the node we are connected to.

            When the connection is made, the method
            outbound_node_connected is invoked.

            If reconnect is True, the node will try to reconnect
            to the code whenever the node connection was closed.
            The method returns True when the node is connected
            with the specific ip."""

        # 수정: 원래 코드는 outbound 접속 성공일 경우에 저장
        #   --> 설정된 모든 노드를 포함하도록 변경
        #   by Giseop 23.6.27.
        # If reconnection to this ip is required, it will be added to the list!
        if reconnect:
                self.debug_print("connect_with_node: Reconnection check is enabled on node " + ip + ":" + str(port))
                self.reconnect_to_nodes.append({
                    "ip": ip, "port": port, "tries": 0
                })
        ...
        try:
            ...
            # 연결 대기시간이 너무 길어서 추가함, by Giseop 23.6.22.(목)
            sock.settimeout(2.0)
            self.debug_print("connecting to %s port %s" % (ip, port))
            ...
            for node in self.nodes_inbound:
                if node.ip == ip and node.id == connected_node_id:
                    # print(f"connect_with_node: This node ({node.ip}:{node.port}) is already connected with us.")
                    sock.send("CLOSING: Already having a connection together".encode('utf-8'))
                    sock.close()
                    return True

            thread_client = self.create_new_connection(sock, connected_node_id, ip, port)
            thread_client.start()
    ...
    def disconnect_with_node(self, node):
        """Disconnect the TCP/IP connection with the specified node.
           It stops the node and joins the thread.

           The node will be deleted from the nodes_outbound list.
           Before closing, the method node_disconnect_with_outbound_node
           is invoked."""
        if node in self.nodes_outbound:
            self.node_disconnect_with_outbound_node(node)
            # 노드 리스트 삭제 추가 by Giseop 2023.6.21.Thu.
    ...
    def reconnect_nodes(self):
        ...
        # print(f'\nreconnect_to_nodes: {self.reconnect_to_nodes}')
        for reconnect in self.reconnect_to_nodes:

            # 수정: 현재 접속된 노드 목록에 없으면 reconnect로 수정, by Giseop 23.6.27
            # print(f'nodes for reconnect: {self.reconnect_to_nodes}')
            # print(f'all nodes: {[f"{x.ip}:{x.port}" for x in self.all_nodes]}')
            ip, port = reconnect.get('ip'), reconnect.get('port')
            # print(f'reconnect node   : {ip}:{port}')
            connected_exist = False
            for node in self.nodes_outbound:
                # print(f'node in all_nodes: {node.ip}:{node.port}')
                if node.ip==ip and node.port==port:
                    connected_exist=True
                if node.ip==self.external_ip and node.port==port:
                    if self.external_ip == self.ip:
                       connected_exist = True
                # print(f'external_ip: {self.external_ip}')

            if not connected_exist:
                reconnection = self.connect_with_node(reconnect["ip"], reconnect["port"]) # Perform the actual connection
                if not reconnect:
                    time.sleep(3)
                # msg = 'success!' if reconnection else 'fail...'
                # print(f"reconnect: {reconnect.get('ip')}:{reconnect.get('port')} --> {msg}")

            # for node in self.all_nodes:
            #     # print(f'utils.get_external_ip(): {utils.get_external_ip()}')
            #     if node.ip != reconnect.get('ip') and node.port == reconnect.get('port'):
            #         reconnection = self.connect_with_node(reconnect["ip"], reconnect["port"]) # Perform the actual connection
            #         msg = 'success!' if reconnection else 'fail...'
            #     if node.ip == reconnect.get('ip') and node.port != reconnect.get('port'):
            #         reconnection = self.connect_with_node(reconnect["ip"], reconnect["port"]) # Perform the actual connection
        ...
        #     self.debug_print("reconnect_nodes: Checking node " + node_to_check["ip"] + ":" + str(node_to_check["port"]))
            # for node in self.nodes_outbound:
            #     if node.ip == node_to_check["ip"] and node.port == node_to_check["port"]:
            #         found_node = True
            #         node_to_check["tries"] = 0 # Reset the trials
            #         self.debug_print("reconnect_nodes: Node " + node_to_check["ip"] + ":" + str(node_to_check["port"]) + " still running!")

            # if not found_node: # Reconnect with node
            #     node_to_check["tries"] += 1
            #     if self.node_reconnection_error(node_to_check["ip"], node_to_check["port"], node_to_check["tries"]):
            #         self.connect_with_node(node_to_check["ip"], node_to_check["port"]) # Perform the actual connection

            #     else:
            #         self.debug_print("reconnect_nodes: Removing node (" + node_to_check["ip"] + ":" + str(node_to_check["port"]) + ") from the reconnection list!")
    ...
    def outbound_node_connected(self, node):
        """This method is invoked when a connection with
           a outbound node was successfull.
           The node made the connection itself."""
    ...
    def node_request_to_stop(self):
        """This method is invoked just before we will stop.
           A request has been given to stop the node and close
           all the node connections.
           It could be used to say goodbey to everyone."""
        self.debug_print("node is requested to stop!")
        if self.callback is not None:
            self.callback("node_request_to_stop", self, {}, {})

    def node_reconnection_error(self, ip, port, trials):
        ...
        self.debug_print("node_reconnection_error: Reconnecting to node " + ip + ":" + str(port) + " (trials: " + str(trials) + ")")
        return True
    ...
    def __repr__(self):
        return '<Node {}:{} id: {}>'.format(self.ip, self.port, self.id[:10])
